...
2. 解析 `path`：绝对路径直接用，相对路径拼工作区；`stat` 失败返回 `grep: path not found`。
   - 是文件：只搜这一个文件，`glob` 参数被忽略。
   - 是目录：用 `fs.glob` 按 `glob` 模式遍历（`dot: false`，隐藏文件不进结果），套用
     [共用忽略规则](./README.md#忽略规则搜索类工具共用)，结果按路径排序。
3. 逐个文件读入（`utf8`），跳过：非普通文件、大于 512 KiB 的文件、内容里含 `\0` 的
   二进制文件、读失败的文件。
4. 按行匹配，累计命中数，达到 `limit` 就停止遍历后续文件。
5. `context > 0` 时把每条命中的 `[i-context, i+context]` 收成互不重叠的区间，相邻区间
   也合并，避免同一行被打印两遍。

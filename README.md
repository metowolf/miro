...
| `/compact [text]` | Summarize earlier context while keeping recent history; optional text guides the summary |
...
Visible thinking blocks retain only metadata; Miro model-context checkpoints also omit raw reasoning and thinking blocks. Raw `agent_thought_chunk` messages are also omitted from ACP traffic logs by default; set `"recordRawThinking": true` in `~/.miro/settings.json` only when those sensitive details are intentionally needed for debugging.
...
Use `/compact` while idle, or `/compact preserve the migration decisions` to guide the summary. It uses the current model, keeps the visible transcript, and saves the compacted model context for session resume. It works even when automatic compaction is disabled. Press `Esc` to cancel without replacing history; insufficient history is a no-op. Pause an active goal first. Print mode also supports `miro -c <session-id> -p "/compact"`, with progress on stderr. For ACP, the command is forwarded only when the provider advertises support.
...
Miro saves model-context checkpoints alongside the visible transcript, including compaction summaries and complete tool-call/result pairs. Resume uses the latest valid checkpoint rather than replaying the pre-compaction transcript; sessions without checkpoints fall back to visible messages. Checkpoints contain conversation and tool data, so keep session files private.
